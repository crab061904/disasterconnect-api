...
// Note: Using firestore and adminAuth imports from User.js
// Assuming this file correctly imports from '../firebaseAdmin.js'
// We rely on the hybrid fix in firebaseAdmin.js to provide 'firestore' and 'db'
import { firestore, auth as adminAuth, db } from "../firebaseAdmin.js"; // Added 'db' for clarity
import { BaseController } from "./BaseController.js";
import { User } from "../models/index.js"; 
// Assuming Organization model is imported correctly, possibly via index.js if using { User }
...
export const authController = {
	...
	async register(req, res) {
		try {
			// Destructure fields, including the specific ones from the frontend
			const { 
				email, 
				password, 
				name, 
				...
				// These are the old, specific field names that may be missing/undefined
				currentLocation,        
				skills,                 
				availability,           
				proofOfVolunteerStatus, 
				organizationName,       
				organizationType,       
				contactInformation,     
				proofOfLegitimacy,      
				
				// This object contains the clean data from the frontend (name, type, contact)
				profileData = {} 
			} = req.body;
			...
			if (role === 'civilian') {
				extraData = {
					location: { 
						address: currentLocation || profileData.location || "",
						lat: null, 
						lng: null 
					}
				};
			...
			// --- VOLUNTEER LOGIC ---
				...
				// Use profileData for volunteer fields if available, otherwise fallback
				const volunteerSkills = profileData.skills || skills;
				const volunteerAvailability = profileData.availability || availability || "Available";

				const skillsArray = Array.isArray(volunteerSkills) 
					? volunteerSkills
					: (volunteerSkills ? volunteerSkills.split(',').map(s => s.trim()) : []);
			...
			// --- ORGANIZATION LOGIC ---
				...
				// Get clean data from profileData (set by the fixed frontend)
				const orgName = profileData.name;
				const orgType = profileData.type;
				const orgContact = profileData.contact;
				
				finalDisplayName = orgName || name; // Use Org Name as main display name

				// ✅ FIX: Explicitly convert potential 'undefined' values to 'null' for Firestore
				extraData = {
					organizationDetails: {
						name: orgName || null,                   
						type: orgType || null,
						contact: orgContact || null,
						proofOfLegitimacy: proofOfLegitimacy || profileData.proofOfLegitimacy || "",
			...
			const userData = { 
				...
				...extraData,
				// ...profileData // We avoid spreading profileData here to prevent duplicates
			};
			...
			// In case of error, show the specific message in the console for better debugging
